...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None, ortho_n_fix=False, maml=False, memQ=None):
        B = x.shape[0]
        ...
        for i,blk in enumerate(self.blocks):
            ...
            if prompt is not None:
                if train:
                    p_list, loss, x = prompt.forward(q, i, x, train=True, task_id=task_id, ortho_n_fix=ortho_n_fix, maml=maml, memQ=memQ)
                    prompt_loss += loss
                else:
                    p_list, _, x = prompt.forward(q, i, x, train=False, task_id=task_id, ortho_n_fix=ortho_n_fix, maml=maml, memQ=memQ)
